...
for u in Us
    for σ in Ωfs
        ξN, Jx = squeezing(U, Ωf, tfs)
        data_name_ξN = "/home/manueloid/Desktop/Ueda_" * string(U) * "_" * string(U * N / (2 * Ωf)) * ".dat"
        data_name_Jx = "/home/manueloid/Desktop/Wineland_" * string(U) * "_" * string(U * N / (2 * Ωf)) * ".dat"
        sqs = todecibel.(ξN * (N / 4)) ./ Jx
        writedlm(data_name_ξN, ξN)
        writedlm(data_name_Jx, sqs)
    end
end

filename_ξN(U, Ωf) = "/home/manueloid/Desktop/Ueda_" * string(U) * "_" * string(U * N / (2 * Ωf)) * ".dat"
filename_Jx(U, Ωf) = "/home/manueloid/Desktop/Wineland_" * string(U) * "_" * string(U * N / (2 * Ωf)) * ".dat"
U, Ωf = 0.1, 0.1
ξN = readdlm(filename_ξN(U, Ωf))
Jx = readdlm(filename_Jx(U, Ωf))
...
canvas = @pgf Axis({
    ...
    # {}, Plot(css_ibjj_style, Table(tfs * U, ξN[:, 3])),
)
plot_name = "/home/manueloid/Desktop/Ueda_zoomX" * string(U) * "_" * string(U * N / (2 * Ωf)) * ".pdf"
